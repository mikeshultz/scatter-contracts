...
def test_accept(web3, contracts):
    """ Test accepting bids """
    _, bidder, hoster, _, _, joe = get_accounts(web3)
    bid = contracts.get(BID_CONTRACT_NAME)
    ...
    # Accept
    accept_hash = bid.functions.accept(bid_id).transact(get_std_tx(hoster))
    ...
    accept2_hash = bid.functions.accept(bid_id).transact(get_std_tx(joe))
    ...
    accept3_hash = bid.functions.accept(bid_id).transact(get_std_tx(joe))
...
def test_pinned(web3, contracts):
    """ Test a simple bid with minimum validations set """
    _, bidder, hoster, _, _, jake = get_accounts(web3)
    bid = contracts.get(BID_CONTRACT_NAME)
    ...
    accept2_hash = bid.functions.accept(bid_id).transact(get_std_tx(jake))
    accept2_receipt = web3.eth.waitForTransactionReceipt(accept2_hash)
    assert accept2_receipt.status == 1, "accept failed"
    assert has_event(bid, 'AcceptWait', accept2_receipt), "AcceptWait event not found"

    # Pin
    pin_hash = bid.functions.pinned(bid_id).transact(get_std_tx(hoster))
...
def test_validation(web3, contracts):
    ...
    - 
    """
    validator4, bidder, hoster, validator1, validator2, validator3 = get_accounts(web3)
    bid = contracts.get(BID_CONTRACT_NAME)
    env = contracts.get(ENV_CONTRACT_NAME)
    ...
    # Validation #1
    v1_hash = bid.functions.validate(bid_id).transact(get_std_tx(validator1))
    ...
    v2_hash = bid.functions.invalidate(bid_id).transact(get_std_tx(validator2))
    ...
    # Validation #3 (valid)
    v3_hash = bid.functions.validate(bid_id).transact(get_std_tx(validator3))
    ...
    # Validation #4 (valid)
    v4_hash = bid.functions.validate(bid_id).transact(get_std_tx(validator4))
